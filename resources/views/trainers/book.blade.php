    ...
    <div class="public-container">
        ...
        <div class="mt-8 grid gap-10 lg:grid-cols-[.75fr_1.25fr]">
            <aside class="h-fit overflow-hidden rounded-[2rem] border border-white/10 bg-[#111411]">
                <div class="grid aspect-[4/3] place-items-center bg-gradient-to-br from-lime-300/35 via-emerald-900 to-black">@if ($trainer->photo_path)<img src="{{ str_starts_with($trainer->photo_path, 'http') ? $trainer->photo_path : Storage::url($trainer->photo_path) }}" alt="{{ $trainer->user->name }}" class="h-full w-full object-cover">@else<span class="text-6xl font-black text-white/25">{{ collect(explode(' ', $trainer->user->name))->map(fn ($part) => mb_substr($part, 0, 1))->take(2)->implode('') }}</span>@endif</div>
