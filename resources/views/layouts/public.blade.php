    ...
    >
        <div class="mx-auto flex min-h-16 max-w-[1600px] items-center justify-between rounded-[1.4rem] bg-white px-4 shadow-[0_12px_40px_rgba(9,20,16,.08)] sm:px-6 xl:min-h-20 xl:px-8">
            ...
            <nav class="hidden items-center gap-0.5 text-sm font-bold text-[#44534d] xl:flex" aria-label="Primary navigation">
                ...
                <x-public-nav-link :href="route('notices.index')" :active="request()->routeIs('notices.*')">Notice Board</x-public-nav-link>
                ...
                <div x-data="{ expanded: false }" x-on:mouseenter="expanded = true" x-on:mouseleave="expanded = false" x-on:click.outside="expanded = false" x-on:keydown.escape.stop="expanded = false" x-on:focusin="expanded = true" x-on:focusout="if (! $el.contains($event.relatedTarget)) expanded = false" class="relative">
                    <div class="flex items-center rounded-full {{ request()->routeIs('yoga-therapy.*', 'therapy-finder.*', 'therapy-appointments.*') ? 'bg-[#10231d] text-white' : 'hover:bg-[#f1f3ef] hover:text-[#10231d]' }}">
                        <a href="{{ route('yoga-therapy.index') }}" class="py-2.5 pl-3">Therapy Services</a>
                        <button type="button" x-on:click.stop="expanded = true" x-bind:aria-expanded="expanded" aria-controls="desktop-therapy-menu" aria-label="Toggle Therapy Services menu" class="py-2.5 pl-1 pr-3"><svg viewBox="0 0 20 20" fill="none" stroke="currentColor" class="h-3.5 w-3.5 transition" x-bind:class="expanded && 'rotate-180'"><path d="m5 7.5 5 5 5-5" stroke-width="1.8"/></svg></button>
                    </div>
                    <div id="desktop-therapy-menu" x-show="expanded" x-transition x-cloak class="absolute left-1/2 top-full z-20 mt-3 w-80 -translate-x-1/2 rounded-[1.5rem] bg-white p-3 text-[#10231d] shadow-[0_20px_55px_rgba(9,20,16,.16)]">
                        <a href="{{ route('yoga-therapy.index') }}#therapy-categories" class="block rounded-xl px-4 py-2 hover:bg-[#f1f3ef]">Cupping Therapy</a>
                        <a href="{{ route('yoga-therapy.index') }}#therapy-categories" class="block rounded-xl px-4 py-2 hover:bg-[#f1f3ef]">Full Body Relaxation</a>
                        <a href="{{ route('yoga-therapy.index') }}#therapy-categories" class="block rounded-xl px-4 py-2 hover:bg-[#f1f3ef]">Deep Tissue Massage</a>
                        <a href="{{ route('yoga-therapy.index') }}#therapy-categories" class="block rounded-xl px-4 py-2 hover:bg-[#f1f3ef]">Trigger Point Release</a>
                        <a href="{{ route('yoga-therapy.index') }}#therapy-categories" class="block rounded-xl px-4 py-2 hover:bg-[#f1f3ef]">Relaxa for Neck, Back, Shoulder &amp; Muscle Pain</a>
                        <a href="{{ route('yoga-therapy.index') }}#therapy-categories" class="block rounded-xl px-4 py-2 hover:bg-[#f1f3ef]">Foot Massage</a>
    ...
    <footer class="bg-[#efefeb] px-3 pb-3 pt-12 text-[#10231d] sm:px-5 sm:pt-16">
        <div class="mx-auto max-w-[1600px] overflow-hidden rounded-[2rem] bg-[#10201a] text-white sm:rounded-[2.75rem]">
            <div class="grid gap-12 px-6 py-14 sm:px-10 lg:grid-cols-[1.25fr_.75fr_.8fr_1fr] lg:px-14 lg:py-20 xl:px-20">
                ...
                <div><p class="footer-heading text-white/40">Quick links</p><nav class="footer-links text-white/70" aria-label="Footer quick links"><a href="{{ route('home') }}">Home</a><a href="{{ route('about.index') }}">About</a><a href="{{ route('notices.index') }}">Notice Board</a><a href="{{ route('programs.index') }}">Programs</a><a href="{{ route('group-programs.index') }}">Group programs</a><a href="{{ route('events.index') }}">Other events</a><a href="{{ route('trainers.index') }}">Our team</a><a href="{{ route('contact.index') }}">Contact</a></nav></div>
                <div><p class="footer-heading text-white/40">Services</p><nav class="footer-links text-white/70" aria-label="Footer service links"><a href="{{ route('trainers.index') }}">Personal training</a><a href="{{ route('group-programs.index') }}">Group programs</a><a href="{{ route('yoga-therapy.index') }}">Therapy services</a><a href="{{ route('therapy-finder.index') }}#specialists">Therapist</a><a href="{{ route('therapy-finder.index') }}">Find your therapy</a><a href="{{ route('products.index') }}">Fitness store</a></nav></div>
